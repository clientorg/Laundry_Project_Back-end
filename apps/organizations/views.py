...
@extend_schema(
    ...
    description=(
        "List organizations or create an organization with "
        "its initial plan and user."
    ),
    request=OrganizationCreateSerializer,
)
class OrganizationListCreateView(PermissionRequiredMixin, generics.ListCreateAPIView):
...
class PlanListCreateView(PermissionRequiredMixin, generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        if not self.request.user.is_superuser:
            raise PermissionDenied("Only super administrators can create plans.")
...
class OrganizationChangePlanView(PermissionRequiredMixin, APIView):
    ...
    def post(self, request, pk):
        ...
        organization = get_object_or_404(Organization, pk=pk, parent__isnull=True)
        ...
        try:
            with transaction.atomic():
                ...
                organization = Organization.objects.create(
                    name=org_data["name"], created_by=user, updated_by=user, parent=None
                )
                ...
                orgadmin_group_detail = GroupDetail.objects.filter(
                    organization=organization, group__name__startswith="org_admin_"
                ).first()
